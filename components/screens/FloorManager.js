...
export default class Screen2 extends Component {
	...
	render() {
	    this.props.itemsRef.on('value', (snap) => {
	    	...
	    	if (!isMounted)
		    	return;
		    // Clear floorSectionsArrayTemp and floorSectionsTemp, and initialize max and rowCount to 0
	      	var floorSectionsArrayTemp = [];
	      	floorSectionsTemp = [];
	      	var max = 0;
	      	var rowCount = 0;
  			...
  			snap.child(selectedProjectKey).child(selectedProjectTitle).child('floorSections').forEach((section) => { // push index so there's unique key
  				for (var i = 0; i < section.val().numRows; i++) { // For each row in section
  					floorSectionsArrayTemp.push(section.val().width); // Push given row to floorSectionsArrayTemp
  				}
  			...
  			// Save floorSectionsArrayTemp and floorSectionsTemp to floorSectionsArray amd rows, respectively
  			this.setFloorSectionsArray(floorSectionsArrayTemp.slice());
