...
export default class Screen3 extends Component {
	    ...
	    itemsRef.on('value', (snap) => {
	    	...
	    	if (!isMounted)
  			...
  			snap.child(selectedProjectKey).child(selectedProjectTitle).child('boards').forEach((board) => {
  				...
  				if (board.key != 'default') { // If board is not the 'default' placeholder board
		    ...
		    this.setState({
		        dataSource: this.state.dataSource.cloneWithRows(items) // save boards in dataSource
		    }, () => {
		    	this.props.setBoards(this.state.dataSource); // save boards in shared 'boards' variable
		    });
	    });
	}
	...
	renderItem(item) {
		// Settings to determine behaviour when board is swiped	
		const swipeSettings = {
			autoClose: true,
